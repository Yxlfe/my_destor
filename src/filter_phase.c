...
static void* filter_thread(void *arg) {
    ...
    while (1) {
        ...
        for (; iter != end; iter = g_sequence_iter_next(iter)) {
            ...
            if (!CHECK_CHUNK(c, CHUNK_DUPLICATE) 
				...
				//container剩下的大小不够chunk的大小时，即container满了时
 ...
 //       segmentid sid = append_segment_flag(jcr.bv, CHUNK_SEGMENT_START, s->chunk_num);
		segmentid sid = -1;
		if (destor.rewrite_algorithm[0] != REWRITE_CAPPING_LOCALITY) {
			//原版destor使用的函数
			sid = append_segment_flag(jcr.bv, CHUNK_SEGMENT_START, s->chunk_num);
		}else {
			//考虑局部性时使用的函数
			sid = append_segment_flag_forLocality(jcr.bv, CHUNK_SEGMENT_START, s->chunk_num);
		}
        	...
        	if(r == NULL){
				...
				*/
				if (destor.rewrite_algorithm[0] != REWRITE_CAPPING_LOCALITY) {
					...
					cp.next = c->next;
					//将块的配方信息写入segmentbuf
					append_n_chunk_pointers_forLocality(jcr.bv, &cp, 1);
				}
 ...
 //      	append_segment_flag(jcr.bv, CHUNK_SEGMENT_END, 0);
		if (destor.rewrite_algorithm[0] != REWRITE_CAPPING_LOCALITY) {
			...
			//考虑局部性时使用的函数
			append_segment_flag_forLocality(jcr.bv, CHUNK_SEGMENT_END, 0);
		}
