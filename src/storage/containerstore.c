...
//自己加的，为了方便调整container.pool文件结构
static int32_t firstContainerOffset = 4 * 1024 * 1024;
//static int32_t firstContainerOffset = 8;
static FILE* fp;
...
void init_container_store() {
	...
	//container.pool文件是由 8byte(记录container_count) + N * 4M(N个容器)
...
void write_container(struct container* c) {
	...
	if (destor.simulation_level < SIMULATION_APPEND) {
		//data区域创建的时候是分配了4M的大小，但是填数据块时最多只填到4M-META_SIZE
		...
		//为适应ceph基本读写单元为4M 大小，将container.pool前4M只写入container_count数据
		if (fseek(fp, c->meta.id * CONTAINER_SIZE + firstContainerOffset, SEEK_SET) != 0) {
		...
		if(fseek(fp, c->meta.id * CONTAINER_META_SIZE + firstContainerOffset, SEEK_SET) != 0){
			perror("Fail seek in container store.");
			exit(1);
		}
...
/*
根据起始容器id和读取个数读取指定范围的容器, array_len将读取的实际个数返回
返回container指针
*/
struct container* retrieve_container_multiple(containerid id, int num, int* array_len) {
	struct container *c = (struct container*) malloc(num * (sizeof(struct container)));
	*array_len = num;
	int i;
	for (i = 0; i < num; i++) {
		init_container_meta(&c[i].meta);
	}

	if (destor.simulation_level == SIMULATION_NO) {
		/* When destor.simulation_level == SIMULATION_NO
		*/
		for (i = 0; i < num; i++) {
			c[i].data = malloc(CONTAINER_SIZE);
		}
		pthread_mutex_lock(&mutex);
		//计算需要读取的容器相对于container.pool文件开始的偏移量，然后寻道
		fseek(fp, id * CONTAINER_SIZE + firstContainerOffset, SEEK_SET);
		for (i = 0; i < num; i++) {
			fread(c[i].data, CONTAINER_SIZE, 1, fp);
		}
		pthread_mutex_unlock(&mutex);
	}
	...
	unsigned char *cur = 0;
	for (i = 0; i < num; i++) {
		cur = &c[i].data[CONTAINER_SIZE - CONTAINER_META_SIZE];
		//开始反序列化容器的元数据
		unser_declare;
		unser_begin(cur, CONTAINER_META_SIZE);

		unser_int64(c[i].meta.id);
		unser_int32(c[i].meta.chunk_num);
		unser_int32(c[i].meta.data_size);
		printf("meta.id is %ld==============\n", c[i].meta.id);


		if (c[i].meta.id != id + i) {
			WARNING("expect %lld, but read %lld", id + i, c[i].meta.id);
			assert(c[i].meta.id == id + i);
		}
		int j;
		for (j = 0; j < c[i].meta.chunk_num; j++) {
			...
			g_hash_table_insert(c[i].meta.map, &me->fp, me);
		}
...
*/
	printf("finished once retrieve_container_multiple()!\n");
	return c;
}
...
struct container* retrieve_container_by_id(containerid id) {
	...
	TIMER_END(1, jcr.system_sync_drop_caches);
	...
	if (destor.simulation_level >= SIMULATION_RESTORE) {
		...
		if (destor.simulation_level >= SIMULATION_APPEND)
			fseek(fp, id * CONTAINER_META_SIZE + firstContainerOffset, SEEK_SET);
		else
			fseek(fp, (id + 1) * CONTAINER_SIZE - CONTAINER_META_SIZE + firstContainerOffset,
			SEEK_SET);
