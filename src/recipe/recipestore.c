...
void update_backup_version(struct backupVersion *b) {
	...
	fwrite(&b->bv_num, sizeof(b->bv_num), 1, b->metadata_fp);
	fwrite(&b->deleted, sizeof(b->deleted), 1, b->metadata_fp);
	//NOTICE("job %d , b->deleted set to %d", b->bv_num, b->deleted);
...
segmentid append_segment_flag_forLocality(struct backupVersion* b, int flag, int segment_size) {
	...
	//containerID设置为负数 0-flag
	cp.id = 0 - flag;
	//块大小设置为该段具有块的数量，表示后面紧跟的段具有块的数量
	...
	if (flag == CHUNK_SEGMENT_START) {
		...
		b->segmentlen = (sizeof(fingerprint) + sizeof(containerid) + sizeof(int32_t)
	...
	memcpy(b->segmentbuf + b->segmentbufoff, &cp.pre, sizeof(int16_t));
	b->segmentbufoff += sizeof(int16_t);
	memcpy(b->segmentbuf + b->segmentbufoff, &cp.next, sizeof(int16_t));
	b->segmentbufoff += sizeof(int16_t);
...
segmentid append_segment_flag(struct backupVersion* b, int flag, int segment_size){
	...
	//containerID设置为负数 0-flag
	cp.id = 0 - flag;
	//块大小设置为segment_size，表示后面紧跟的段的大小
	cp.size = segment_size;
	memset(&cp.fp, 0, sizeof(cp.fp));
...
void append_n_chunk_pointers_forLocality(struct backupVersion* b,
	...
	int i;
	for (i = 0; i < n; i++) {
		struct chunkPointerForLocality bcp = cp[i];
		...
		b->segmentbufoff += sizeof(int32_t);
		memcpy(b->segmentbuf + b->segmentbufoff, &(bcp.pre), sizeof(int16_t));
		b->segmentbufoff += sizeof(int16_t);
		memcpy(b->segmentbuf + b->segmentbufoff, &(bcp.next), sizeof(int16_t));
		b->segmentbufoff += sizeof(int16_t);
 ...
 * n 为一次读取的chunkPointer个数，k为读取实际读取的chunkPointer个数
 * 返回chunkPointer指针
 */
struct chunkPointer* read_next_n_chunk_pointers(struct backupVersion* b, int n,
	...
	for (i = 0; i < num; i++) {
		...
		//为了使rewrite-capping-locality和普通ASM搭配使用时
		if (destor.restore_cache[0] == RESTORE_CACHE_ASM_LOCALITY) {
			fread(&temp, sizeof(int16_t), 2, b->recipe_fp);
		}
...
struct chunkPointerForLocality* read_next_n_chunk_pointers_forLocality(struct backupVersion* b, 
	...
	struct chunkPointerForLocality *cp = (struct chunkPointerForLocality *) malloc(
		sizeof(struct chunkPointerForLocality) * num);

	for (i = 0; i < num; i++) {
		fread(&(cp[i].fp), sizeof(fingerprint), 1, b->recipe_fp);
		fread(&(cp[i].id), sizeof(containerid), 1, b->recipe_fp);
		fread(&(cp[i].size), sizeof(int32_t), 1, b->recipe_fp);
		fread(&(cp[i].pre), sizeof(int16_t), 1, b->recipe_fp);
		fread(&(cp[i].next), sizeof(int16_t), 1, b->recipe_fp);
		/* Ignore segment boundaries */
		if (cp[i].id == 0 - CHUNK_SEGMENT_START || cp[i].id == 0 - CHUNK_SEGMENT_END)
			i--;
	}
