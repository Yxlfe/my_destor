...
/* 根据需要备份的文件的路径path执行备份 */
void do_backup(char *path) {
	
	//初始化recipe、container、index、job control record(jcr)
	init_recipe_store();
	...
	//带参宏定义，#define TIMER_DECLARE(n) struct timeval b##n,e##n
	TIMER_DECLARE(1);
	//带参宏定义，#define TIMER_BEGIN(n) gettimeofday(&b##n, NULL)
	TIMER_BEGIN(1);

    time_t start = time(NULL);//！！！！貌似这里无用！！！！！
	...
	//查询索引、标记重复的块、更新唯一块的指纹信息到索引
	start_dedup_phase();
	//根据重写算法进行重写。如果是cap：对超出阈值的块标记为CHUNK_OUT_OF_ORDER
	start_rewrite_phase();
	//最后根据flag进行过滤写入数据块,生成recipe文件
	start_filter_phase();
	...
	printf("deduplication ratio (in size): %.4f, %.4f\n",
			jcr.data_size != 0 ?
	...
	printf("deduplication ratio (in number): %.4f, %.4f\n",
			jcr.chunk_num != 0 ?
				(jcr.chunk_num - jcr.unique_chunk_num
					- jcr.rewritten_chunk_num)
					/ (double)(jcr.chunk_num) :
				0,
			jcr.chunk_num
					/ (double)(jcr.unique_chunk_num + jcr.rewritten_chunk_num));
	 ...
	 * deduplication rate(in size)--deduplication rate(in size),
	 * rewritten rate,
	 ...
	 * throughput,
	 */
	fprintf(fp, "%" PRId32 " %" PRId64 " %" PRId64 " %.4f-%.4f %.4f %" PRId32 " %" PRId32 " %" PRId32 " %" PRId32" %" PRId32 " %" PRId32" %" PRId32" %.2f\n",
			jcr.id,
			jcr.data_size,
