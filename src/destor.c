...
void destor_start() {
	...
	//加载解析配置文件destor.config ，并将配置值设入全局变量结构体变量destor(变量在destor.h中定义）
	load_config();	

	sds stat_file = sdsdup(destor.working_directory);

	//将stat_file指向的字符串和字符串"/destor.stat"拼接
...
int main(int argc, char **argv) {
	//初始化设置destor默认值,以及加载配置文件
	destor_start();
	//默认为写入模式，写入命令格式为===== destor /path/to/data
	int job = DESTOR_BACKUP;
	//读取备份job id为revision的文件
	int revision = -1;
	...
	while ((opt = getopt_long(argc, argv, short_options, long_options, NULL))
		...
		switch (opt) {
		case 'r':
			//读取模式，命令格式为===== destor -r0 /path/to/restore (注意这里的0指的是job id 编号)
			job = DESTOR_RESTORE;
		...
		case 't':
			...
			break;
		case 'h':
			//显示空间使用状态，命令格式为 destor -h
			usage();
			break;
		case 'p': {
			//临时添加配置参数,命令格式如为 destor /path/to/data -p"a line of config"
...
struct chunk* new_chunk(int32_t size) {
	...
	ck->physical_address_start = -1;
	ck->physical_address_end = -1;
	ck->pre = -1;
	ck->next = -1;
	ck->role = -1;
