...
static struct trigger_chunk*  new_trigger_chunk(int64_t id, int64_t byte_position) {
	...
	trigger->id = id;
	//printf("===new a trigger_entity ,it's byte position is %lld\n",byte_position);
...
static void set_chunk_data_forASM(gpointer data, gpointer userdata) {
	struct chunk *c = (struct chunk*)data;
	struct container *con = (struct container*)userdata;
	struct chunk *buf = get_chunk_in_container(con, &c->fp);
	assert(c->size == buf->size);
...
static int upa_push(struct chunk* c) {
	...
	jcr.recipe_num++;
	if (jcr.recipe_num > jcr.max_recipe_num)
		jcr.max_recipe_num = jcr.recipe_num;
...
static int transition_area_push() {
	...
	if (flag == 1) {
		...
		if (temp_trigger != NULL) {
			...
			/*
			if(trigger->id == 308){
				printf("======trigger chunk of container is 308, the byte_position is %lld\n", trigger->byte_position);
			}
			*/
		}
		else {
...
static struct chunk* asm_pop(GSequenceIter *begin) {
	...
	/*if(CHECK_CHUNK(c, CHUNK_CACHED) && c->id == 308){
		printf("the chunk position is %lld(B), the trigger->byte_position is %lld(B), when this chunk is trigger chunk and chunk cached!\n", assembly_area.spa_tail_position, trigger->byte_position);
	}*/
	...
	if (trigger != NULL && assembly_area.spa_tail_position == trigger->byte_position) {
		...
		if (CHECK_CHUNK(c, CHUNK_READY)) {
			//将该触发块从该区触发表中删除
			/*if(trigger->id != -1)
				printf("-----remove trigger_chunk entity from spa table, id is %lld, byte_position is %lld \n", trigger->id, trigger->byte_position);*/
		...
		if (CHECK_CHUNK(c, CHUNK_CACHED)) {
			...
			/*printf("chech a chunk is trigger chunk and this chunk cached !!!!!\n");*/
			...
			assert(!CHECK_CHUNK(c, CHUNK_CACHED));
...
static void expel_cache() {
	...
	if (begin != end) {
		...
		/*printf("!!!!will expel a trigger chunk, the position is %lld(B), id is %lld .\n the ASM used_size is %lld , the cache_size will be %lld from %lld .\n",
			trigger->byte_position, trigger->id, assembly_area.used_size, assembly_area.cache_size - trigger->size, assembly_area.cache_size);
		*/
	...
	while (trigger != NULL) {
		if (trigger->is_cached == 0) {
			if (assembly_area.used_size + assembly_area.cache_size +
				...
				/*printf("@@@@ will cache a trigger chunk, the position is %lld(B), id is %lld .\n the ASM used_size is %lld , the cache_size will be %lld from %lld .\n",
					trigger->byte_position, trigger->id, assembly_area.used_size, assembly_area.cache_size + trigger->size, assembly_area.cache_size);
				*/
				...
				if (max_trigger != NULL && trigger->size < max_trigger->size) {
					...
					/*printf("@@@@ will first expel, then cache a trigger chunk, the position is %lld(B), id is %lld .\n the ASM used_size is %lld , the cache_size will be %lld from %lld .\n",
						trigger->byte_position, trigger->id, assembly_area.used_size, assembly_area.cache_size + trigger->size, assembly_area.cache_size);
						*/
...
static GQueue* enhance_assemble_area() {
	...
	if (CHECK_CHUNK(c, CHUNK_CACHED)) {
		...
		/*printf("At %lld(B),max ASM-cache size is %.4f(MB), +++ use cache: container %lld \n", byte_position-c->size, (assembly_area.used_size + c->size )/ 1024.0 / 1024.0, c->id);*/
	...
	VERBOSE("At %lld(B), Restore cache: container %lld is missed", byte_position, id);
	if (destor.simulation_level == SIMULATION_NO || destor.simulation_level == SIMULATION_RESTORE) {
		TIMER_DECLARE(2);
		TIMER_BEGIN(2);
	...
	if (destor.simulation_level == SIMULATION_NO || destor.simulation_level == SIMULATION_RESTORE) {
		...
		g_queue_foreach(trigger->chunk_queue, set_chunk_data_forASM, con);
		assembly_area.used_size += trigger->size;
	}
	...
	/*printf("At %lld(B),max ASM-cache size is %.4f(MB), Restore cache: container %lld is missed \n", byte_position, assembly_area.used_size / 1024.0 / 1024.0, id);*/
