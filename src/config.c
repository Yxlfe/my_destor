...
void load_config_from_string(sds config) {
	...
	for (i = 0; i < totlines; i++) {
		...
		if (strcasecmp(argv[0], "working-directory") == 0 && argc == 2) {
			...
			else if (strcasecmp(argv[1], "capping-locality") == 0)
				destor.rewrite_algorithm[0] = REWRITE_CAPPING_LOCALITY;
			else if (strcasecmp(argv[1], "capping-my") == 0) {
				destor.rewrite_algorithm[0] = REWRITE_CAPPING_MY;
				if (argc > 3) {
					//用在caping-my上的
					destor.rewrite_algorithm[2] = atoi(argv[3]);
					destor.rewrite_algorithm[3] = atoi(argv[4]);
					destor.rewrite_algorithm[4] = atoi(argv[5]);
					destor.rewrite_algorithm[5] = atoi(argv[6]);
				}
			}else if(strcasecmp(argv[1], "capping-double") == 0){
				destor.rewrite_algorithm[0] = REWRITE_CAPPING_DOUBLESIZE;
				if (argc > 3) {
					//用在caping-double上的
					destor.rewrite_algorithm[2] = atoi(argv[3]);
					destor.rewrite_algorithm[3] = atoi(argv[4]);
			...
			else if (strcasecmp(argv[1], "capping-fcrc") == 0) {
				destor.rewrite_algorithm[0] = REWRITE_CAPPING_FCRC;
				if (argc > 3) {
					//用在caping-fcrc上的
					destor.rewrite_algorithm[2] = atoi(argv[3]);
					destor.rewrite_algorithm[3] = atoi(argv[4]);
			...
			}else if (strcasecmp(argv[1], "capping-lbw") == 0) {
				destor.rewrite_algorithm[0] = REWRITE_CAPPING_LBW;
				if (argc > 3) {
					//用在caping-lbw上的
					destor.rewrite_algorithm[2] = atoi(argv[3]);
					destor.rewrite_algorithm[3] = atoi(argv[4]);
		...
		} else if (strcasecmp(argv[0], "restore-cache") == 0 && argc >= 4) {
			if (strcasecmp(argv[1], "lru") == 0) {
				destor.restore_cache[0] = RESTORE_CACHE_LRU;
				destor.restore_cache[3] = atoi(argv[3]);
			}
			...
			else if (strcasecmp(argv[1], "asm-locality") == 0){
				destor.restore_cache[0] = RESTORE_CACHE_ASM_LOCALITY;
				destor.restore_cache[3] = atoi(argv[3]);
			}
			else if (strcasecmp(argv[1], "my-asm") == 0) {
				destor.restore_cache[0] = RESTORE_CACHE_MY_ASM;
				destor.restore_cache[2] = atoi(argv[3]);
				destor.restore_cache[3] = atoi(argv[4]);
			}
			else{
